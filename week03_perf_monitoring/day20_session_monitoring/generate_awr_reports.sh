...
sqlplus -s "$DB_CONN" @sql/create_workload.sql

# 2. Take starting AWR snapshot
BEGIN_SNAP=$(sqlplus -s "$DB_CONN" <<EOF
...
sqlplus -s "$DB_CONN" @sql/run_load.sql

# 4. Take ending snapshot
END_SNAP=$(sqlplus -s "$DB_CONN" <<EOF
SET PAGESIZE 0 FEEDBACK OFF VERIFY OFF HEADING OFF ECHO OFF
SELECT MAX(snap_id) FROM dba_hist_snapshot;
EXIT;
EOF
...
sqlplus -s "$DB_CONN" @sql/get_sql_ids.sql
...
sqlplus -s "$DB_CONN" @sql_tuning_advisor.sql $TOP_SQL | tee $OUT_DIR/tuning_report.txt
